...
<!-- Canonical copy: `.ai-assist/`. Root `AGENTS.md`, `code-standards.md`, `agents`, and `CLAUDE.md` symlink here. -->

## Syncing this bundle into another repo

1. From this repo: `npm run sync-to-project -- --target /path/to/other/project`
2. After clone/CI: if `.ai-assist/meta.json` is missing or clearly stale vs this repo’s `package.json` version, re-run the same command (or `node /path/to/ai_assist/scripts/sync-to-project.mjs --target ...`).

## Project Overview

- Describe the product domain and high-level architecture.
- List subprojects and their responsibilities.
- Note TypeScript usage across subprojects.
...
- Subproject-specific rules may exist in `.cursor/rules` within subprojects (optional extensions)
...
- Communication style: Talk like a caveman! Rules: 
  - No filler/preamble
  - Run tools first, show the result, then stop. Do not narrate.
  - Use short 3-10 word sentences. 
...
- **Throw errors on failures, don't return null** - Services, repositories, and job processors should throw on errors, not return null (return null only for "not found" scenarios). Workflow/report orchestration may complete partially, but must surface a user-visible signal (e.g., `PARTIAL_FAILURE` or "Content incomplete") and document the decision.
- **Always log errors in catch blocks** - Never have empty catch blocks; log with context before re-throwing
- **Git add discipline**: Never use `git add -a`, `git add -A`, or add untracked files indiscriminately unless explicitly asked. When asked to add/commit, stage only files related to the current work.
- **When adding env vars**: Update the shared secrets config (if any) and README.md
...
- **Migrations**: Use the project’s migration generation command when available; avoid hand-writing migrations unless documented.

## Project Structure & Module Organization

- Frontend app: UI pages, components, hooks, shared utilities, and tests.
- Backend app: routers/controllers, services, repositories, shared types/utilities, and DB schema/migrations.
- Infrastructure: stacks, deploy manifests, and monitoring assets.
...
- **Orchestration**: Job/queue operations belong in orchestration layers, not repositories
- **Split large files**: If a file exceeds ~500 lines, split into smaller focused modules

## Build, Test, and Development Commands

- Document standard dev/build/test commands for each subproject.
...
- Follow ESLint defaults: React components PascalCase, hooks prefixed with `use`, and route folders lowercase (if applicable). Backend modules prefer camelCase.
...
- **ALWAYS reference `code-standards.md`** when writing code for detailed type safety guidelines

## Testing Guidelines

- Frontend tests should use the standard test runner and live near code in `__tests__` directories.
- Backend tests should use the standard test runner with fixtures near test suites.
...
- **⚠️ IMPORTANT: Before committing, check if you're on the main branch. If so, suggest to user to create a feature branch first. Never commit directly to main/master branches without approval. If a branch is already merged, suggest to create a new branch. When working as a cloud agent always start a new branch after merging.** 
...
- PRs should summarise intent, link tickets, attach screenshots or sample payloads for UI/API changes, and request review from the squad.
...
- Store secrets in per-service `.env` files (never commit).
- Infrastructure edits must flow through infrastructure-as-code; console changes should be avoided.

### Adding New Environment Variables

When introducing new env vars, update ALL of these:

1. Shared secrets config (if any) for production/staging deployment
2. Environment-specific deploy overrides if needed
3. `.env.example` or document in README for local development
4. Centralized env validation with sensible defaults
...
- All infrastructure changes must go through infrastructure-as-code
...
- File reads are allowed by default; "changes" means modifications to files, config, or repo state.
...
- When updating patterns, also update `code-standards.md` to maintain consistency
