...
- If substantial work is present (multiple file changes or more than a couple of requirements/acceptance criteria) and no clear spec/plan/intent is found, raise a `MINOR` issue recommending a `spec.md` be added in `docs/plans/...` as part of the commit.
...
> **Note**: All checklist items should be evaluated against `code-standards.md` for detailed standards and examples.
...
- **Config**:
  - New env vars added to shared secrets config (if any) AND `.env.example`
  - Env vars validated in centralized env validation
- **Job Processing**:
  - Job processors throw on failure (never return null), unless partial completion is explicitly documented with warning logs and status/metrics
  - Job data not mutated directly (use supported update APIs)
  - Correlation ID in job logs
- **Queue Configuration**:
  - No queue-level configuration without documented reason
  - Backoff configuration is intentional and documented
  - Bulk enqueue is used where supported
- **Code Style**:
  - No nested ternaries
...
- **Typing** – Strong TypeScript types, no `any`. Prefer shared types or schemas where applicable. Discourage complex inline types that should be named exports.
  ...
  - **Shared types**: Reuse existing shared types where applicable, or create new shared types if the type is used across multiple modules
...
- **Database & Migrations** – Confirm schema changes have safe defaults, back-fill data where necessary, maintain indexes/constraints, and update ORM models.
...
- **Backend (TypeScript/API)** – Enforce thin routers, ensure business logic lives in services/orchestration; validate request parsing and DTO typing; confirm new schema/migrations keep referential integrity, default values, and are idempotent; verify repositories guard against tenant leakage and handle transactions/rollback paths; check job processors for retry safety and correlation IDs.
- **Frontend (React)** – Ensure components follow framework conventions (server/client boundaries where applicable); confirm hooks use existing API clients and caching conventions; review form state management for controlled inputs and validation; verify utility usage stays consistent and responsive; ensure API calls properly handle loading/error states and match backend DTOs; confirm test coverage and accessibility affordances (labels, ARIA, keyboard support).
...
- **Redundant Implementation** – Check if the new code duplicates existing functionality:
  ...
  - Check for map functions that are offering no value (selecting all fields)
- **AI Code Quality & Complexity** – Actively seek out signs of AI slop and high cyclomatic complexity (see `code-standards.md` section "AI Code Quality & Complexity"):
...
2. `## Intent Alignment` – Surface gaps between requirements and implementation. **Only list items with problems** – do not list files that are fully aligned.
   ...
   Example rows:
   - `src/services/foo.ts` | ⚠️ Partial | Implements retry logic not in spec | Clarify: intended or scope creep?
...
4. `## Decisions Needed` – List ambiguous or underspecified items that require product or engineering direction before implementation or classification. Number each item sequentially. Include a short question and the impact on scope or correctness if unresolved.

5. `## Changed/Lost Functionality` – List behavior changes where intent is unclear (including removals or output differences). Number each item sequentially. For each, include: **Before**, **After**, **Evidence**, **Risk**, **Decision Needed**.

6. `## Issues` – One subsection per finding, sorted by severity (highest first). Number each issue sequentially. Include all issues. Do not just show the top 3 or 4. It's fine to show 10+ issues if there are that many. Follow this template:
   ...
   **Issue Type**: Indicate the nature of the issue in the title using one of these prefixes:
   - `[REGRESSION]` – Breakage of previously working behavior or a lost capability
   - `[BUG]` – Confirmed incorrect behavior or broken functionality (non-regression)
   ...
   **Detecting Pattern vs Standards Conflicts**: When reviewing code:
   ...
   - **Compare with standards**: Check `code-standards.md` for documented standards
   ...
   **Problem Section Guidance**: When writing the Problem section:
   - If the issue relates to standards alignment, explicitly state whether it violates `code-standards.md` and cite the specific standard
   - If there's a conflict between existing patterns and standards, highlight both:
     - "This violates `code-standards.md:X` which requires Y, but the existing codebase pattern (see `file1.ts`, `file2.ts`) uses Z instead"
...
7. `## Missing Tests` – List specific tests that should be written, organized by layer:
...
8. `## PR Quality Checklist` – Summarize results from running `agents/pr-checklist.md`:
   ...
   | Job Processing | ✅ Pass / ⚠️ Issues / N/A | Brief note |
...
9. `## Positive Observations` – Optional. Call out well-executed patterns or improvements.
10. `## Follow-ups` – List clarifications needed, blocked verifications, or suggested future work that are not decision blockers.
...
- **Issue completeness**: Do not limit the number of issues shown per severity level. Show ALL issues found, especially all CRITICAL, MAJOR and MINOR issues without exception.
