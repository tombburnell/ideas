...
| ---------------------------- | ------------------------------ | ------------------------------------------------------ | --------------------------------- |
| `getById(id)`                | Fetch entity with related data | Join base + type-specific tables                        | `feature-repo.ts:45-80`           |
| `updateStatus(id, status)`   | Update entity status           | Update base table, set `completedAt` if COMPLETED       | `feature-repo.ts:195-203`         |
| `upsertAnalysis(id, output)` | Store analysis results         | Varies by feature                                       | See related repo                  |
```
...
❌ **Bad**: "Extend `AbstractRepository` pattern for a new feature repository"
...
- Complex workflows (e.g., job queue flows)
...
These tests MUST use the test container setup (integration test utilities) with a real database:
...
**Test Container Setup**: See integration test helpers (e.g., `tests/integration/shared.ts`) for:
...
- **Use test containers** — they spin up a real database via containers
...
| Test | File | Reason |
|------|------|--------|
| Tenant isolation in `getAll` | `repository.test.ts` | Security: prevent cross-org data leakage |
| FK SET NULL on delete | `repository.test.ts` | Related records must survive delete |
| NULL unique constraint | Requires real DB | COALESCE index can't be tested with mocks |
````
