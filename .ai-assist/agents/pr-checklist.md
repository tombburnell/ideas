...
- `REGRESSION` for breakages of previously working behavior or lost capabilities
- `BUG` for confirmed incorrect behavior that is not a regression
...
- [ ] **1.1** Services and job processors throw errors on failure (not return null), unless partial completion is explicitly documented with warning logs and status/metrics
...
- [ ] **1.5** Workflow/report orchestration may complete partially, but must surface a user-visible signal (e.g., `PARTIAL_FAILURE` or "Content incomplete")
...
**Common violation**: Business logic or queue operations in repository files.
...
- [ ] **4.1** New env vars added to shared secrets config (if any)
- [ ] **4.2** New env vars documented in `.env.example` or README
- [ ] **4.3** Env vars have centralized validation with sensible defaults
...
- [ ] **5.1** Job processors throw on failure (never return null), unless partial completion is explicitly documented with warning logs and status/metrics
- [ ] **5.2** Job data updates use provided APIs rather than mutating raw data
- [ ] **5.3** Correlation ID is included in all job-related logs
- [ ] **5.4** Jobs are idempotent (safe to retry)

**Common violation**: Mutating job data directly without using supported APIs.

### 6. Queue Configuration & Job Optimization

- [ ] **6.1** No queue-level configuration without good reason and comments
- [ ] **6.2** Backoff configuration is intentional and documented
- [ ] **6.3** Bulk enqueue is used where supported instead of per-item enqueue in loops

**Common violation**: Using per-item enqueue loops when bulk options exist.

### 7. Type Safety

- [ ] **7.1** No `any` types (use explicit types or `unknown` with guards)
- [ ] **7.2** Complex inline types extracted to named interfaces
- [ ] **7.3** Enums used for finite value sets (not string literals)
...
### 10. Regressions & Changed Functionality

- [ ] **10.1** Regression check: existing behavior still works as before
- [ ] **10.2** If behavior changed or outputs differ, the change is clearly intentional and documented
- [ ] **10.3** If intent is unclear, call it out as Changed/Lost Functionality for review

**Common violation**: Silent behavior change without documented intent.
...
- Full coding standards: `code-standards.md`
